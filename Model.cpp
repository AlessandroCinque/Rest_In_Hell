	...
	while (!file.eof())
	{
		...
		if (subStrings[0] == "v" || subStrings[0] == "vn")
		{
			...
			/*if (subStrings[0] == "v")
			{
				...
				vertexArray.push_back(glm::vec3((GLfloat)(atof(subStrings[index].c_str())),
					(GLfloat)(atof(subStrings[index + 1].c_str())),
					(GLfloat)(atof(subStrings[index + 2].c_str()))));
			}*/
			if (subStrings[0] == "v")
			{
...
void Model::Create()
{
	...
	GLuint shaderProgram = TheShader::Instance()->GetShaderProgram();
	...
	m_textureFlagID = glGetUniformLocation(shaderProgram, "is_textured");
...
void Model::Update()
{
	...
	TheShader::Instance()->SendUniformData("time", time);
...
void Model::Draw()
{
	...
	//convert model matrix to 3x3 and invert it for normals to use in shader
	m_normal = glm::inverse(glm::mat3(m_model));
	//TheShader::Instance()->SendUniformData("model", m_model);
	
	if (!Hitted) 
	{
		TheShader::Instance()->SendUniformData("isDemon", true);
		TheShader::Instance()->SendUniformData("isDamaged", false);
	}
	else 
	{
		TheShader::Instance()->SendUniformData("isDemon", false);
		TheShader::Instance()->SendUniformData("isDamaged", true);
	}
